...
from schemas_format.course_schemas import (CoursePriceRequestInSchema, CourseRequestInSchema, CourseRequestOutSchema,
                                           CourseRequestOutOptionSchema, ExamDateDirectorRequestInSchema, ExamDateRequestInSchema, SeminarRequestInSchema,
                                           SeminarRequestOutSchema, ExamDateRequestOutSchema, ExamDateDirectorRequestOutSchema, SeminarRequestMutipleInSchema,
                                           SubjectRequestInSchema, SubjectCourseRequestInSchema, SubjectRequestOutSchema,
                                           SubjectRequestOutOptionSchema, SubjectCourseRequestOutSchema)
...
def get_subject(school_id: str, only: bool, request: FilterRequestSchema, db: Session = Depends(get_db),  authenticated: bool = Depends(auth_request)):
    ...
    if only == True:
        ...
        queryset = or_(Subject.school_id == school_id,
                       Subject.school_id == base_school_id)
    else:
        queryset = Subject.school_id == school_id
    searchFilter = or_(Subject.subject_code.contains(search_value),
                       Subject.subject_name.contains(search_value))

    if search_value:
        result = db.query(Subject).options(joinedload(Subject.school_subject)).order_by(desc(Subject.create_date)).filter(
            Subject.cancelled == 1, queryset, searchFilter).offset(skip).limit(limit).all()
    else:
        result = db.query(Subject).options(joinedload(Subject.school_subject)).order_by(desc(Subject.create_date)).filter(
            Subject.cancelled == 1, queryset).offset(skip).limit(limit).all()
    total_data = db.query(Subject).filter(
        Subject.cancelled == 1, queryset).count()
    total_filter_data = len(result)
    total_page = ceil(total_data / request.per_page)
    return SubjectRequestOutOptionSchema(status="success", status_code="200", message="Success fetch all data", page=request.page, per_page=limit, total_page=total_page, total_data=total_data, total_filter_data=total_filter_data, data=result)
...
def get_course(school_id: str, only: bool, request: FilterRequestSchema, db: Session = Depends(get_db),  authenticated: bool = Depends(auth_request)):
    if only == True:
        # โชว์หลักสูตรกลางและของตนเอง
        queryset = or_(Course.school_id == school_id,
                       Course.school_id == base_school_id)
    else:
        queryset = Course.school_id == school_id
    search_value = request.search_value
    searchFilter = or_(Course.course_code.contains(search_value),
                       Course.course_name.contains(search_value))
    # skip = ternaryZero(request.page - 1)
    skip = ternaryZero(((request.page - 1) * request.per_page))
    limit = request.per_page

    if search_value:
        result = db.query(Course).options(joinedload(Course.school_course)).order_by(desc(Course.create_date)).filter(Course.cancelled == 1,
                                                                                                                      queryset, searchFilter).offset(skip).limit(limit).all()
    else:
        result = db.query(Course).options(joinedload(Course.school_course)).order_by(desc(Course.create_date)).filter(
            Course.cancelled == 1, queryset).offset(skip).limit(limit).all()
    total_data = db.query(Course).filter(
        Course.cancelled == 1, queryset).count()
    total_filter_data = len(result)
    total_page = ceil(total_data / request.per_page)
    return CourseRequestOutOptionSchema(status="success", status_code="200", message="Success fetch all data", page=request.page, per_page=limit, total_page=total_page, total_data=total_data, total_filter_data=total_filter_data, data=result)
...
def create_seminar(request: SeminarRequestInSchema, db: Session = Depends(get_db), authenticated: bool = Depends(auth_request)):
    ...
    db.commit()
    db.refresh(_seminar)
    return ResponseProcess(status="success", status_code="200", message="Success created data | seminar_id =  " + str(_seminar.seminar_id))
...
def get_by_seminar_id(seminar_id: int,  db: Session = Depends(get_db), authenticated: bool = Depends(auth_request)):
    _seminar = db.query(Seminar).options(joinedload(Seminar.subject_seminar),
                                         ...
                                         joinedload(Seminar.school_seminar)).filter(
        Seminar.seminar_id == seminar_id).one_or_none()
    if not _seminar:
        raise HTTPException(status_code=404, detail="Data not found")
    return _seminar
...
def get_seminar(school_id: str, course_id: str,   month: int, year: int, branch_id: str = "all",   db: Session = Depends(get_db),  authenticated: bool = Depends(auth_request)):
    ...
    if branch_id != 'all':
        ...
        queryset = Seminar.branch_id == branch_id
    else:
        queryset = Seminar.school_id == school_id
    ...
    _subject = db.query(Seminar).options(joinedload(Seminar.subject_seminar),
        ...
        Seminar.cancelled == 1, Seminar.course_id == course_id,  extract('year', Seminar.seminar_date) == year, extract('month', Seminar.seminar_date) == month, queryset, ).all()
...
def get_examdate(school_id: str, vehicle_type_id: int, month: int, year: int, branch_id: str = "all",  db: Session = Depends(get_db),  authenticated: bool = Depends(auth_request)):

    if branch_id != 'all':
        # โชว์เฉพาะสาขาตนเอง
        queryset = ExamDate.branch_id == branch_id
    else:
        queryset = ExamDate.school_id == school_id
      # แสดงข้อมูลโดยอิงประเภทยานพาหนะ
    if vehicle_type_id != 0:
        # โชว์วิชาอบรมเฉพาะสาขาตนเอง
        queryset2 = ExamDate.vehicle_type_id == vehicle_type_id
    else:
        queryset2 = ExamDate.vehicle_type_id > 0

    _examdate = db.query(ExamDate).options(joinedload(ExamDate.branch_examdate), joinedload(ExamDate.school_examdate)).order_by(asc(ExamDate.ed_date)).filter(
        ExamDate.cancelled == 1, extract('year', ExamDate.ed_date) == year, extract('month', ExamDate.ed_date) == month, queryset, queryset2).all()
