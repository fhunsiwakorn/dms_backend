...
def get_user(request: FilterRequestSchema, typeuser: str = "all", school_id: str = "all", branch_id: str = "all", db: Session = Depends(get_db),  authenticated: bool = Depends(auth_request)):
    ...
    if school_id == "all":
        filterA = User.school_id != school_id
    else:
        filterA = User.school_id == school_id

    if branch_id == "all":
        filterB = User.branch_id != branch_id
    else:
        filterB = User.branch_id == branch_id
    # Filter user type : all , 1,2,3,4,5

    if typeuser == "1" or typeuser == "2" or typeuser == "3" or typeuser == "4" or typeuser == "5":
        filter = User.user_type == int(typeuser)
    else:
        filter = User.user_type > 0

    searchFilter = or_(User.firstname.contains(search_value),
    ...
    if search_value:
        result = db.query(User).order_by(desc(User.create_date)).options(joinedload(User.branch_user), joinedload(
            User.school_user)).filter(User.cancelled == 1, filter, filterA, filterB, searchFilter).offset(skip).limit(limit).all()
    else:
        result = db.query(User).order_by(desc(User.create_date)).options(joinedload(User.branch_user), joinedload(User.school_user)).filter(User.cancelled == 1,
                                                                                                                                            filter, filterA, filterB).offset(skip).limit(limit).all()
    total_data = db.query(User).filter(
        User.cancelled == 1, filter, filterA, filterB).count()
