...
def general_location(request: FilterRequestSchema, db: Session = Depends(get_db)):
    ...
    # ค้นหาปกติ
    searchFilter = or_(LocationThai.zipcode.contains(search_value),
                       LocationThai.amphur_name.contains(search_value),
                       LocationThai.district_name.contains(search_value),
    ...
    if search_value:
        ...
        _location = db.query(LocationThai).offset(skip).limit(limit).all()
    total_data = db.query(LocationThai).count()
    total_filter_data = len(_location)
...
def general_location_filter(request: locationFilter, db: Session = Depends(get_db)):
    # ค้นหาปกติ
    searchFilter = and_(LocationThai.amphur_name.contains(request.amphur_name),
                        LocationThai.district_name.contains(
                            request.district_name),
                        LocationThai.province_name.contains(request.province_name))
    _location = db.query(LocationThai).filter(
        searchFilter).one_or_none()
    if not _location:
